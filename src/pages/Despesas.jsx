    ...
    return (
      <div style={{ padding:'8px 0', borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ display:'flex', gap:'6px', marginBottom:'6px' }}>
          <input value={local.nome} onChange={e=>setLocal({...local,nome:e.target.value})} placeholder="Nome"
            style={{ flex:2, background:'rgba(255,255,255,0.06)', border:`1px solid ${catCor}30`, borderRadius:'7px', padding:'6px 8px', color:'#e2e8f0', fontSize:'12px' }} />
          <input type="number" value={local.valor} onChange={e=>setLocal({...local,valor:e.target.value})} placeholder="R$"
            style={{ flex:1, background:'rgba(255,255,255,0.06)', border:`1px solid ${catCor}30`, borderRadius:'7px', padding:'6px 8px', color:'#e2e8f0', fontSize:'12px', fontFamily:"'JetBrains Mono',monospace" }} />
        </div>
        <input value={local.observacao} onChange={e=>setLocal({...local,observacao:e.target.value})} placeholder="Observação"
          style={{ width:'100%', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'7px', padding:'5px 8px', color:'#e2e8f0', fontSize:'11px', marginBottom:'6px' }} />
...
function CategoriaCard({ cat, index, onAddItem, onUpdateItem, onRemoveItem }) {
  ...
  const addItem = () => {
    ...
    setNovoItem({ nome:'', valor:'', observacao:'' })
    setAddingItem(false)
  }

  return (
    <div style={{ background:'linear-gradient(135deg,#111827,#0d1422)', border:`1px solid ${cat.cor}20`, borderRadius:'20px', overflow:'hidden', marginBottom:'10px', animation:`fadeUp 0.5s ease ${0.1+index*0.05}s both` }}>
      ...
      {open && (
        <div style={{ padding:'0 18px 16px', borderTop:'1px solid rgba(255,255,255,0.04)', animation:'scaleIn 0.2s ease both' }}>
          <div style={{ paddingTop:'6px' }}>
            {cat.itens.map(item => (
              <ItemRow key={item.id} item={item} catId={cat.id} catCor={cat.cor} onUpdate={onUpdateItem} onRemove={onRemoveItem} />
            ))}
          </div>
          {cat.nome==='Variáveis' && (
          ...
          {addingItem ? (
            <div>
              <div style={{ display:'flex', gap:'6px', marginBottom:'6px' }}>
                <input autoFocus value={novoItem.nome} onChange={e=>setNovoItem({...novoItem,nome:e.target.value})} placeholder="Nome do item"
            ...
            <button onClick={()=>setAddingItem(true)} style={{ width:'100%', background:`${cat.cor}07`, border:`1px dashed ${cat.cor}22`, borderRadius:'8px', padding:'7px', color:cat.cor, fontSize:'12px', cursor:'pointer' }}>
              + Adicionar item
            </button>
          )}
        </div>
...
export default function Despesas() {
  ...
  const pieData = categoriasDespesas.map(c => {
    ...
    return {
      ...
      cor: c.cor,
      icone: c.icone,
      pct: totalMensal>0?((valor/totalMensal)*100).toFixed(1):0,
    }
  ...
  return (
    <div style={{ padding:'0 0 8px' }}>
      ...
      <div style={{ background:'linear-gradient(160deg,#0d1526,#070b16)', padding:'48px 20px 20px', position:'relative', overflow:'hidden' }}>
        ...
        <h1 style={{ fontSize:'26px', fontWeight:800, letterSpacing:'-0.02em', animation:'fadeUp 0.5s ease both' }}>Despesas</h1>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px', marginTop:'14px', animation:'fadeUp 0.5s ease 0.1s both' }}>
          {[
            { label:'Total Mensal', value:fmt(totalMensal), color:'#f87171' },
            { label:'Total Anual', value:fmt(totalAnual), color:'#fb923c' },
          ].map(s => (
            <div key={s.label} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'12px', padding:'12px', textAlign:'center' }}>
              <p style={{ fontSize:'18px', fontWeight:800, color:s.color, fontFamily:"'JetBrains Mono',monospace" }}>{s.value}</p>
      ...
      <div style={{ padding:'16px' }}>
        ...
        <div style={{ background:'#111827', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'20px', padding:'18px', marginBottom:'16px', animation:'fadeUp 0.5s ease 0.15s both' }}>
          <p style={{ fontSize:'13px', fontWeight:600, color:'rgba(226,232,240,0.7)', marginBottom:'12px' }}>Distribuição por Categoria</p>
          <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
            <ResponsiveContainer width={150} height={150}>
              <PieChart>
                <Pie data={pieData} cx={70} cy={70} innerRadius={36} outerRadius={68} dataKey="value" paddingAngle={2}>
                  {pieData.map((entry,i) => <Cell key={i} fill={entry.cor} stroke="transparent" />)}
                </Pie>
