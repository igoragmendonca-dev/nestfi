...
const SUPABASE_URL = 'https://dugjdgubdtsewiyaxusw.supabase.co'
const SUPABASE_KEY = 'sb_publishable_UzUS6THYTQ_2svjHnSL8_g_OHo5dF8W'
const TABLE = 'nestfi_data'
...
function debouncedSave(state) {
  ...
  saveTimeout = setTimeout(() => {
    ...
    for (const key of Object.keys(state)) {
      if (typeof state[key] !== 'function') clean[key] = state[key]
    }
    salvarNoSupabase(clean)
  }, 1500)
}
const DEFAULTS = {
  salario: { salarioBasico:15414.88, rmnr:43678.25, anuenio:2867.1676, gratFuncaoGer:2302.59, heDesembarque:1662.3363, auxilioEducacional:962.0, inss:988.07, ams:462.4464 },
  outrasRendas: [
    { id:1, nome:'Aluguel Imóvel SP', tipo:'simples', valor:3200, despesas:[{id:11,nome:'IPTU rateado',valor:120},{id:12,nome:'Administração (6%)',valor:192}], descricao:'Apartamento Moema', recorrente:true, categoria:'Aluguel' },
    { id:2, nome:'Dividendos / FIIs', tipo:'simples', valor:2100, despesas:[], descricao:'BOVA11 + HGLG11 + MXRF11', recorrente:false, categoria:'Investimentos' },
    { id:3, nome:'Freelance Consultoria', tipo:'simples', valor:5000, despesas:[{id:31,nome:'Carnê-leão IR (~15%)',valor:750}], descricao:'Projetos pontuais', recorrente:false, categoria:'Freelance' },
  ],
  investimentos: [
    ...
    { id:12, tipo:'contaInvestimento',nome:'Conta XP',            valor:35000,  rentabilidadeAnual:10.5, descricao:'Conta remunerada XP',      corretora:'XP' },
    { id:13, tipo:'fgts',             nome:'FGTS Acumulado',      valor:68000,  rentabilidadeAnual:3.0,  descricao:'TR + 3% ao ano',           corretora:'CEF' },
    { id:14, tipo:'previdencia',      nome:'Petros BD',           valor:320000, rentabilidadeAnual:8.5,  descricao:'Previdência complementar', corretora:'Petros' },
  ...
  categoriasDespesas: [
    { id:1, nome:'Fixos', cor:'#6366f1', icone:'🏠', itens:[
    ...
    { id:2, nome:'Essenciais', cor:'#10b981', icone:'🛒', itens:[
    ...
    { id:3, nome:'Crianças', cor:'#f59e0b', icone:'👶', itens:[
    ...
    { id:4, nome:'Variáveis', cor:'#ec4899', icone:'🎭', itens:[
  ...
  financiamentos: [
    { id:1, nome:'Apartamento Leblon', tipo:'imovel', saldoDevedor:680000, valorOriginal:900000, taxaJuros:9.5, prazoMeses:240, parcelasMeses:180, parcelaMensal:6900, amortizacao:'SAC', dataInicio:'2010-06-01', custos:[{nome:'Seguro MIP',valor:180},{nome:'Taxa Admin',valor:25}], observacao:'CEF — TR + 9,5% a.a.' },
    { id:2, nome:'Porsche Cayenne 2023', tipo:'veiculo', saldoDevedor:285000, valorOriginal:380000, taxaJuros:12.5, prazoMeses:48, parcelasMeses:24, parcelaMensal:8200, amortizacao:'PRICE', dataInicio:'2023-01-01', custos:[{nome:'Seguro Auto',valor:850}], observacao:'Banco Porsche Financial Services' },
  ],
  bens: [],
  marcos: [
...
const useFinancasStore = create(
  persist(
    (set, get) => ({
      ...
      carregarDaCloud: async () => {
        const dados = await carregarDoSupabase()
        if (dados && Object.keys(dados).length > 0) {
          set({ ...dados, _sincronizado: true })
        } else {
          debouncedSave(get())
          set({ _sincronizado: true })
        }
      },

      updateSalario: (campo, valor) => { set((s) => ({ salario: { ...s.salario, [campo]: parseFloat(valor)||0 } })); debouncedSave(get()) },

      addOutraRenda: (renda) => { set((s) => ({ outrasRendas: [...s.outrasRendas, { ...renda, id:Date.now(), despesas:[] }] })); debouncedSave(get()) },
      updateOutraRenda: (id, dados) => { set((s) => ({ outrasRendas: s.outrasRendas.map((r) => r.id===id ? {...r,...dados} : r) })); debouncedSave(get()) },
      removeOutraRenda: (id) => { set((s) => ({ outrasRendas: s.outrasRendas.filter((r) => r.id!==id) })); debouncedSave(get()) },
      addDespesaRenda: (rendaId, despesa) => { set((s) => ({ outrasRendas: s.outrasRendas.map((r) => r.id===rendaId ? {...r, despesas:[...(r.despesas||[]),{...despesa,id:Date.now()}]} : r) })); debouncedSave(get()) },
      removeDespesaRenda: (rendaId, despesaId) => { set((s) => ({ outrasRendas: s.outrasRendas.map((r) => r.id===rendaId ? {...r, despesas:(r.despesas||[]).filter(d=>d.id!==despesaId)} : r) })); debouncedSave(get()) },

      addInvestimento: (inv) => { set((s) => ({ investimentos: [...s.investimentos, {...inv, id:Date.now()}] })); debouncedSave(get()) },
      updateInvestimento: (id, dados) => { set((s) => ({ investimentos: s.investimentos.map((i) => i.id===id ? {...i,...dados} : i) })); debouncedSave(get()) },
      removeInvestimento: (id) => { set((s) => ({ investimentos: s.investimentos.filter((i) => i.id!==id) })); debouncedSave(get()) },

      addCategoria: (cat) => { set((s) => ({ categoriasDespesas: [...s.categoriasDespesas, {...cat, id:Date.now(), itens:[]}] })); debouncedSave(get()) },
      updateCategoria: (id, dados) => { set((s) => ({ categoriasDespesas: s.categoriasDespesas.map((c) => c.id===id ? {...c,...dados} : c) })); debouncedSave(get()) },
      removeCategoria: (id) => { set((s) => ({ categoriasDespesas: s.categoriasDespesas.filter((c) => c.id!==id) })); debouncedSave(get()) },
      addItemDespesa: (catId, item) => { set((s) => ({ categoriasDespesas: s.categoriasDespesas.map((c) => c.id===catId ? {...c, itens:[...c.itens,{...item,id:Date.now()}]} : c) })); debouncedSave(get()) },
      updateItemDespesa: (catId, itemId, dados) => { set((s) => ({ categoriasDespesas: s.categoriasDespesas.map((c) => c.id===catId ? {...c, itens:c.itens.map((i) => i.id===itemId ? {...i,...dados} : i)} : c) })); debouncedSave(get()) },
      removeItemDespesa: (catId, itemId) => { set((s) => ({ categoriasDespesas: s.categoriasDespesas.map((c) => c.id===catId ? {...c, itens:c.itens.filter((i) => i.id!==itemId)} : c) })); debouncedSave(get()) },

      addAssinatura: (ass) => { set((s) => ({ assinaturas: [...s.assinaturas, {...ass, id:Date.now(), ativa:true}] })); debouncedSave(get()) },
      updateAssinatura: (id, dados) => { set((s) => ({ assinaturas: s.assinaturas.map((a) => a.id===id ? {...a,...dados} : a) })); debouncedSave(get()) },
      removeAssinatura: (id) => { set((s) => ({ assinaturas: s.assinaturas.filter((a) => a.id!==id) })); debouncedSave(get()) },
      toggleAssinatura: (id) => { set((s) => ({ assinaturas: s.assinaturas.map((a) => a.id===id ? {...a, ativa:!a.ativa} : a) })); debouncedSave(get()) },

      addFinanciamento: (fin) => { set((s) => ({ financiamentos: [...s.financiamentos, {...fin, id:Date.now()}] })); debouncedSave(get()) },
      updateFinanciamento: (id, dados) => { set((s) => ({ financiamentos: s.financiamentos.map((f) => f.id===id ? {...f,...dados} : f) })); debouncedSave(get()) },
      removeFinanciamento: (id) => { set((s) => ({ financiamentos: s.financiamentos.filter((f) => f.id!==id) })); debouncedSave(get()) },

      addBem: (bem) => { set((s) => ({ bens: [...(s.bens||[]), {...bem, id:Date.now()}] })); debouncedSave(get()) },
      updateBem: (id, dados) => { set((s) => ({ bens: (s.bens||[]).map((b) => b.id===id ? {...b,...dados} : b) })); debouncedSave(get()) },
      removeBem: (id) => { set((s) => ({ bens: (s.bens||[]).filter((b) => b.id!==id) })); debouncedSave(get()) },

      updateMarco: (id, dados) => { set((s) => ({ marcos: s.marcos.map((m) => m.id===id ? {...m,...dados} : m) })); debouncedSave(get()) },
    }),
